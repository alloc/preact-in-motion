...
} from './internal/currentComponent'
...
hook(PrivateHook.Render, storeCurrentComponent)
...
hook('diffed', (vnode: VNode) => {
  diffAnimateProp(vnode)
  popCurrentComponent(vnode)
})
...
function diffAnimateProp(vnode: VNode) {
  const dom = getElementForVNode(vnode)
  if (!dom) {
    return
  }
